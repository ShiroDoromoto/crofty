...
func TestRunReport(t *testing.T) {
	...
	rep, err := c.RunReport("123456789", GA4Query{
	...
	if rep.Property != "123456789" || rep.RowCount != 2 || len(rep.Rows) != 2 {
	...
	if _, err := c.RunReport("123456789", GA4Query{Metrics: []string{"sessions"}, Start: "7daysAgo", End: "today"}); err != nil {
...
func TestSearchAnalytics(t *testing.T) {
	...
	c := NewClient(testCreds(t, srv.URL+"/token"))
	rep, err := c.SearchAnalytics("sc-domain:example.com", GSCQuery{
	...
	if !strings.Contains(gotPath, "sc-domain%3Aexample.com") {
	...
	if rep.Site != "sc-domain:example.com" || len(rep.Rows) != 1 {
		t.Fatalf("report = %+v", rep)
	}
	r0 := rep.Rows[0]
...
func TestSitesAndSitemaps(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(r.URL.Path, "/sites"):
			fmt.Fprint(w, `{"siteEntry":[{"siteUrl":"sc-domain:example.com","permissionLevel":"siteFullUser"}]}`)
		case strings.HasSuffix(r.URL.Path, "/sitemaps"):
			fmt.Fprint(w, `{"sitemap":[{"path":"https://example.com/sitemap.xml","isPending":false}]}`)
		default:
			t.Errorf("unexpected path %s", r.URL.Path)
		}
	}))
