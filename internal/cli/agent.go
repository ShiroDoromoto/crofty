...
type brief struct {
	...
	// ReleaseNotesURL is where the changes are written up, so the AI can tell the
	// author what an upgrade would bring instead of only that one exists. Present
	// only when there is an upgrade to weigh.
...
func agentBrief() brief {
	...
	// The notes URL rides along only when there is an upgrade to weigh; on a
	// current binary it would be a link to nothing the author needs to read.
	notesURL := ""
	if newer {
	...
	return brief{
		...
		Workflow: []string{
			...
			"crofty preview — see it locally in a browser (no account). Bare `crofty preview` blocks, so to keep working use `crofty preview --detach`: it backgrounds the server itself and returns once the URL answers (never background it yourself with shell tricks). Then END it with `crofty preview stop` when the author is done looking (check with `crofty preview status --json`); it also auto-stops after --timeout (default 30m) and only one runs per project, so a forgotten preview can't pile up. Once the author sees it, offer to change the look: the layout is frozen so it can't break, but the design is theirs — a ready-made preset (`crofty theme set quiet-paper` / `terminal`), then individual tokens (colour / type / reading-width via `crofty theme tokens`), and `crofty theme eject` as the full escape hatch. Don't assume the default is final.",
			"crofty deploy — build the current site and publish it to the configured backend: Cloudflare Pages by default, or the author's own server over SFTP/FTPS (set at `crofty init --provider …` or switched later with `crofty connect --provider …`). deploy builds first, so a stale ./dist can't ship; `crofty build` alone is just to inspect ./dist. It publishes static files only — a project carrying Pages Functions is stopped rather than quietly stripped of them (see notes).",
		},
		Commands: cmds,
		Pages: pageGuide{
		...
		Inspect: []string{
			...
			"crofty config --json     — this project now: title, languages, features on, theme, deploy target, footer credit, crofty's state directory (where it is, whether crofty may write there), and pagesFunctions (what at the root answers requests — whether a deploy can carry it depends on the provider; see notes)",
			...
			"crofty doctor --json     — check the built ./dist against the output contract (the gate before deploy), plus deploy: whether a run with no terminal could publish from here (credential source, and what is missing with the fix for each), and selfUpdate: whether `crofty update` will work from this install (route, writable)",
		},
		Notes: []string{
			...
			"A deploy with no terminal never guesses which Cloudflare account to publish to: it stops non-zero and names what to set. Pin it first — the author deploys once from their own terminal, which writes `deploy.accountId` into `.crofty/config.json` (a file that ships with the site), or the run passes `--account <id>`. Ask before you deploy rather than find out by deploying: `crofty doctor --json` → deploy answers whether a run with no terminal has what it needs here, and lists what is missing with the fix for each. It reads presence only — that a credential is there, never that it still works — so a deploy can still fail on a token the destination rejects.",
			...
			"`crofty preview` runs a blocking local server; `crofty preview --detach` starts the same server in the background and returns once it answers, writing its output to .crofty/preview.log. Use --detach rather than backgrounding crofty yourself — no `&`, no `start`, no `Start-Process`. Either way you own ending it: `crofty preview stop` when the author is done (idempotent — safe even if none is running), `crofty preview status --json` to check. It also auto-stops after --timeout (default 30m) — a deadline crofty honours on the next run in this project even if the preview process was killed — and starting a second reaps the first, so only one ever runs per project. Prefer stopping it explicitly over leaving it to the timeout; `--timeout 0` opts out of the deadline entirely, so only use it when the author asked for a preview that runs until they stop it.",
			...
			"crofty runs Hugo for `build` and `preview`. The click installers (.pkg / .exe) carry the Hugo crofty was tested against, and crofty prefers that copy over anything on PATH; every other install uses `hugo` from PATH, which must be the extended build. On Windows there is a third way when none is there: `crofty update` replaces the install-script binary with the one that carries Hugo — worth offering, since Hugo publishes no extended build for Windows on ARM, so installing one is not always possible. If build fails on the wrong Hugo, the author can name one with `CROFTY_HUGO=/path/to/hugo` — theirs to set, not yours.",
			"crofty updates itself with `crofty update` — it self-fetches the release and swaps the binary in place (no reinstall, no admin), on the routes that can: the macOS .pkg, the Windows installer, and the per-user install.sh. There is no automatic update; the upgrade is always started on purpose. When `updateAvailable` is set, tell the author what changed (releaseNotesURL) so it's their call, then run `crofty update` if they want it — it exits non-zero and explains itself if this install can't self-update (a retired channel, a system-wide install, a source build), where the manual way out still stands. Check `crofty doctor --json` → selfUpdate to know in advance whether it will work here.",
			...
			"crofty carries a finished worker; it never builds one. A `_worker.js` at the project root that is a single self-contained module — no `import`, no `require`, no `export … from` — is deployed to Cloudflare Pages as the deployment's `_worker.bundle`, together with the root `_routes.json` if there is one. crofty sends that file as written and synthesizes no default: without it every request runs the worker, static files included, so a deploy with no `_routes.json` warns and continues. What crofty cannot carry stops the deploy before it builds or authenticates, exiting non-zero: a `functions/` directory (carrying it would mean bundling), a `_worker.js` that imports, and anything request-answering when the provider is sftp/ftps. The worker's runtime is the author's to pin, in `.crofty/config.json` → `deploy.worker.compatibilityDate` (a `YYYY-MM-DD` date): crofty injects no default of its own, because then upgrading crofty would change the runtime their worker executes on. Undeclared, the deploy warns and continues — the Pages project's setting decides, or the oldest runtime if it has none. Suggesting a date to the author is helpful; writing one in unasked is not. The variables the worker needs are declared the same way, in `deploy.worker.requiredEnv` — a list of **names**, never values: crofty compares them against the Pages project and warns about the ones that are not set, so a worker that would have failed one request at a time is caught at deploy time instead. Values never go in that file and crofty has no path that sends one; the author sets them on the destination (Pages → project → Settings → Variables and secrets), and a name written as `NAME=value` is rejected rather than trimmed. A missing variable warns and continues — crofty did not unset it, and refusing would take away the only move left. Read `crofty config --json` → pagesFunctions to know what is at stake in advance. Do NOT reach for `--static-only` on your own — it means \"leave those routes offline on purpose\", which is the author's call, and it drops the worker even where crofty could have carried it. Tell them what would be lost and let them choose; the alternative is deploying the site the way its Functions are deployed (wrangler, or a Pages git build).",
...
func agentDetails() map[string]agentCmd {
	return map[string]agentCmd{
		...
		"preview": {
			Flags: []agentFlag{
				{"--detach", "serve in the background and return once the URL answers — use this instead of backgrounding crofty yourself; output goes to .crofty/preview.log"},
			...
			Examples: []string{
				...
				"crofty preview --detach        # serves in the background; returns once the URL answers",
...
func printAgentBrief(b brief) {
	...
	if b.UpdateAvailable {
		fmt.Printf("update:  crofty %s is available — tell the author what changed (%s), then to run: %s\n",
			b.LatestVersion, b.ReleaseNotesURL, upgradeHint())
	}
	fmt.Println()
