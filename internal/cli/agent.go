...
func agentBrief() brief {
	...
	return brief{
		...
		Inspect: []string{
			...
			"crofty config --json     — this project now: title, languages, features on, theme, deploy target, footer credit, crofty's state directory (where it is, whether crofty may write there), and pagesFunctions (non-empty = deploy stops by default; see notes)",
		...
		Notes: []string{
			...
			"crofty publishes static files only — it never uploads Cloudflare Pages Functions (a `functions/` directory or a root `_worker.js`). If the project has them, `crofty deploy` stops before it builds or authenticates and exits non-zero: deploying would replace whatever serves those routes now, so forms and APIs the site answers today would go quiet while the command looked successful. Read `crofty config --json` → pagesFunctions to know this in advance. When it is non-empty, do NOT reach for `--static-only` on your own — that flag means \"take the Functions offline on purpose\", which is the author's call. Tell them what would be lost and let them choose; the alternative is deploying the site the way its Functions are deployed (wrangler, or a Pages git build).",
